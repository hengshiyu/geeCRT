contMAEE = function (y, X, id, Z, maxiter, epsilon, printrange, alpadj, shrink, makevone) {
	...
	#       called when computing the variance at the end (0 , 1). Prints
	...
	SCORE = function(Ustarold, beta, alpha, y, X, Z, n, p, q, s2,flag, rangeflag, 
		...
		for (i in 1:length(n)) {
			...
			for (j in 1:(n[i] - 1)) {
				...
				for (k in (j + 1):n[i]) {
					...
					if ((gamma_c[l] >= 1) | (gamma_c[l] <= -1) & (flag == 1)) {
		...
		rangeflag = 0
		return(list(U = U, UUtran = UUtran, Ustar = Ustar, flag = flag, rangeflag = rangeflag, 
								VEEFLAG = VEEFLAG, NPSDFLAG = NPSDFLAG, NPSDADJFLAG = NPSDADJFLAG))
	}
	...
	# converge: Did the algorithm converge (0, 1)
	#####################################################################################
		...
		while ((niter <= maxiter) & (max(abs(delta)) > epsilon)) {
			...
			repeat{
				...
				if (rangeflag == 1) {
					...
					n_modi = n_modi + 1
					if (printrange) {
						warning(cat("Iteration", niter, "and Shrink Number", n_modi, "\n"))
					}
				}
	...
	# Final Results
	if (SINGFLAG == 1) {stop("Derivative matrix for beta is singular during updates")}
	if (ROBFLAG == 1) {stop("Sandwich variance is not positive definite")}
	...
	if (converge == 1 & ROBFLAG == 0) {
		...
		outList  = list(outbeta = result$outbeta, outalpha = result$outalpha,
										beta = beta, alpha = alpha, MB = naive, BC0 = robust, BC1 = varKC, 
										BC2 = varMD, BC3 = varFG, niter = niter)
